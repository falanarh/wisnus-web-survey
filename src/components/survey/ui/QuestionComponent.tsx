...
import { getCurrentMonthYear, replacePlaceholders } from '@/app/utils/functions';
import CustomDropdown from './CustomDropdown';

const ADDITIONAL_INFO_CODES = ['KR004', 'KR005', 'KR006', 'S009', 'S011', 'S015', 'S021', 'S023', 'S024', 'S028', 'S029'];
...
const QuestionComponent: React.FC<QuestionProps> = ({ question, darkMode }) => {
    ...
    const isS007Disabled = useMemo(() => {
        return code === 'S007' && (!answers['S006'] || answers['S006'].trim() === '');
    }, [code, answers['S006']]);
    ...
    const renderAdditionalInfo = () => {
        ...
        return (
            <>
                ...
                <div className={`text-sm font-semibold ${darkMode ? 'text-gray-400' : 'text-[#565656]'} my-2`}>
                    <p>{firstLine}</p>
                    {listItems.length > 0 && (
    ...
    const renderInput = () => {
        ...
        if (type === "select" && multiple && filteredOptions) {
            return (
                <MultiSelectDropdown
                    name={code}
                    options={Array.isArray(filteredOptions) ? filteredOptions.map(opt => typeof opt === 'string' ? opt : opt.text) : []}
                    value={answers[code] || ""}
                    onChange={handleChange}
                    darkMode={darkMode}
                    placeholder={getPlaceholder()}
                ...
                {additional_info && (
                    <>
                        {!ADDITIONAL_INFO_CODES.includes(code) && (
                            <span className={`text-sm font-semibold ml-[5px] ${darkMode ? 'text-gray-400' : 'text-[#565656]'} my-2`}>
                                {"(" + additional_info + ")"}
                                {validation.required && <span className="text-red-500 ml-[5px]">*</span>}
                            </span>
                        )}
